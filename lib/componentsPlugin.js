  ...
  return function (schema, options) {
    ...
    schema.methods[config.authorizedToJSON] =
      function (userId, optionsToJSON, done) {
        ...
        return authorizedToJSON(this, userId, optionsToJSON, [], done);
      };

    function authorizedToJSON (doc, userId, optionsToJSON, visitedDocs, done) {
      ...
      return async.waterfall([
        ...
        function (components, cb) {
          return docToJSON(userId, optionsToJSON, doc, components, visitedDocs,
                           cb);
        }
      ], done);
    }

    function docToJSON (userId, optionsToJSON, doc, readComponents, visitedDocs,
                        done) {
      ...
      function processObj (obj, path, cbObj) {
        ...
        function processLeaf(obj, objdoc, pathOptions, schema, cbLeaf) {
          ...
          if (schema) {
            return docToJSON(userId, optionsToJSON, objdoc, readComponents,
          ...
          componentResolve(pathOptions.component, doc,
            ...
            if (pathOptions.ref) {
              ...
              // populated
              return authorizedToJSON(objdoc, userId, optionsToJSON,
                                      _.clone(visitedDocs, true), cbLeaf);
            }
    ...
    /*
     * Checks if the provided json can be set by the userId in the provided
     * doc and sets the data. If the userId is not authorized to write a field,
     * the document is not modified at all.
     *
     * Parameters:
     *  * json: input as a plain object. The following values are allowed:
    ...
    schema.methods[config.authorizedSet] = 
      function (json, userId, done) {
        authorizedSet(json, this, userId, done);
      };
    function authorizedSet(json, doc, userId, done) {
      if (!_.isPlainObject(json)) {
        return done(new Error('json must be a plain object'));
      }
      ...
      return async.waterfall([
        ...
        function (writeComponents, cb) {
          // recursively check if the json object can be set on the doc
          return checkJSONSchema(json, doc.schema, doc, writeComponents, cb);
        },
        function (cb) {
          // set the json on the document
    ...
    /* recursively walks the json object and checks if it complies with the 
     * provided document schema and writeComponents
     *
     * Parameters:
     *  * json: see authorizedSet
     ...
     */
    function checkJSONSchema(json, schema, doc, writeComponents, done) {

      checkObj(json, null, done);

      /* recursively walk through objects.
       * (does not process referenced documents)
       * Parameters:
       *  * obj an object or array with data to be stored
       *  * path: current path for json in doc. Either:
            ...
            return function (cb) {
              ...
              if (pathConfig) {
                ...
                if (_.isArray(pathConfig.options.type)) {
                  ...
                  if (pathConfig.options.type[0].paths) {
                    return cb(new Error(
    ...
    schema.methods[config.authorizedArrayPush] =
      function (json, array, userId, done) {
        var doc = this;
          ...
          function (writeComponents, cb) {
            ...
            return componentResolve(
              ...
              function (err, component) {
                ...
                // check JSON with the subdocument's schema
                return checkJSONSchema(json, array._schema.schema, null, 
                                       writeComponents, cb);
              }
            );
          },
          function (cb) {
            // push json to the array
            array.push(json);
            cb();
          }
        ], done);
    ...
    schema.methods[config.authorizedArraySet] =
      function (id, json, array, userId, done) {
        var doc = this;
        ...
        return async.waterfall([
          ...
          function (writeComponents, cb) {
            ...
            return checkJSONSchema(
              json, array._schema.schema, subdoc, writeComponents,
              function (err) {
                if (err) return cb(err);
                cb(null, subdoc);
              }
