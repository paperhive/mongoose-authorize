    ...
    schema.methods.authorizedToJSON = function (userId, done) {
      ...
      return async.waterfall([
        function (cb) {
          ...
          return doc[config.permissionsGetComponents](userId, 'read', cb);
        },
        function (components, cb) {
          return docToJSON(userId, doc, components, cb);
        }
      ], done);
    };

    function docToJSON (userId, doc, readComponents, done) {
      ...
      function processObj (obj, path, cbObj) {
        ...
        function processLeaf(obj, objdoc, pathOptions, schema, cbLeaf) {
          ...
          if (schema) {
            return docToJSON(userId, objdoc, readComponents, cbLeaf);
          }
