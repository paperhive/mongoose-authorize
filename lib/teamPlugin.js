...
module.exports = function (schema, options) {
  ...
  schema.methods[options.methodKey] = function (done) {
    var doc = this;

    // "un-populate" users if necessary in order to get ids
    var user_ids = _.map(doc[options.schemaKey].users, function (value) {
      return value._id ? value._id : value;
    });
    doc.populate(options.schemaKey + '.teams', function (err, doc) {
      if (err) return done(err);
      async.parallel(
        _.map(doc[options.schemaKey].teams, function (team) {
